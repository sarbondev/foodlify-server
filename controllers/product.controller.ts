...
export const createProduct = async (req: Request, res: Response) => {
    ...
    const { title, description, images, previewImage, priceCard, priceRegular, discount, category, weight, stock } =
        req.body

    try {
        const [titleDetect] = await Promise.all([translate(title, { to: "en" })])

        const lang = titleDetect.from.language.iso
        ...
        if (lang === "uz") {
            ...
            return res.status(400).json({ error: "Faqat uz yoki ru tilida matn kiriting" })
        }
